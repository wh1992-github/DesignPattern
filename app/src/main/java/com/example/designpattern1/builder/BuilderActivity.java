...
public class BuilderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.tvDefine.setText(EMTagHandler.fromHtml(Constants.BUILDER_DEFINE));
        //传统Builder模式
        binding.btBuyAodi.setOnClickListener(v -> {
            Director director = new Director();
            Product product = director.getAProduct();
            product.showProduct();
        });
        binding.btBuyBaoma.setOnClickListener(v -> {
            Director director = new Director();
            Product product = director.getBProduct();
            product.showProduct();
        });
        //新型Builder模式
        binding.btSetComputer.setOnClickListener(v -> {
            PersonalComputer computer = new PersonalComputer.Builder("I7处理器", "三星16G")
                    .setDisplay("AOC显示器")
                    .setKeyboard("Logic键盘")
                    .setUsbCount(2)
                    .build();
            LogUtil.i(TAG, "onCreate: PersonalComputer: " + computer.toString());
        });
    }
}
